...
.s-drawer-close:hover { background: var(--s-bg-subtle); color: var(--s-text-primary); }
.s-drawer-body {
    flex: 1;
    overflow-y: auto;
    padding: 20px;
...
focus_active  = "active" if st.session_state.focus_mode else ""
spec_label    = specialty_short.get(specialty_profile, specialty_profile)

st.markdown(f"""
<div class="s-topbar" id="s-topbar">
    <div class="s-logo">Salience <span>OS</span></div>
    <div class="s-spec-badge" onclick="window.parent.document.getElementById('open-drawer-btn').click()"
         title="Click to change specialty">
        ⊕ {spec_label}
    </div>
    <div class="s-topbar-spacer"></div>
    <div class="s-topbar-actions">
        <div id="theme-cycle-display" style="display:flex;gap:4px">
            <button class="s-theme-btn" id="tbtn-light"  onclick="setTheme('light')"  title="Light mode">☀ Light</button>
            <button class="s-theme-btn" id="tbtn-system" onclick="setTheme('system')" title="System">◑ System</button>
            <button class="s-theme-btn" id="tbtn-dark"   onclick="setTheme('dark')"   title="Dark mode">☽ Dark</button>
        </div>
    </div>
</div>
<script>
...
# =====================================================================
# 6. SETTINGS DRAWER (slide-over overlay via components.html)
# =====================================================================
has_cloud_groq   = "groq_api_key"   in st.secrets
has_cloud_gemini = "gemini_api_key" in st.secrets

# Hidden Streamlit button — triggered by JS from the top bar badge
open_drawer_col, close_drawer_col, focus_col = st.columns([1, 1, 1])
with open_drawer_col:
    if st.button("open_drawer", key="open-drawer-btn", label_visibility="collapsed"):
        st.session_state.show_drawer = True
        st.rerun()
with close_drawer_col:
    if st.button("close_drawer", key="close-drawer-btn", label_visibility="collapsed"):
        st.session_state.show_drawer = False
        st.rerun()
with focus_col:
    if st.button("focus_toggle", key="focus-toggle-btn", label_visibility="collapsed"):
        st.session_state.focus_mode = not st.session_state.focus_mode
        st.rerun()

# JS wires the visible top bar buttons to the hidden Streamlit buttons
components.html("""
<script>
(function() {
    function clickParentBtn(key) {
        const btns = window.parent.document.querySelectorAll('button');
        for (const b of btns) {
            if (b.textContent.trim() === key) { b.click(); break; }
        }
    }
    window.__salienceOpenDrawer  = () => clickParentBtn('open_drawer');
    window.__salienceCloseDrawer = () => clickParentBtn('close_drawer');
    window.__salienceFocusToggle = () => clickParentBtn('focus_toggle');

    // Wire top-bar spec badge
    const badge = window.parent.document.querySelector('.s-spec-badge');
    if (badge) badge.onclick = () => clickParentBtn('open_drawer');
})();
</script>
""", height=0, scrolling=False)
...
if st.session_state.show_drawer:
    with st.container():
        st.markdown("""
        <div class="s-drawer-backdrop" onclick="window.__salienceCloseDrawer && window.__salienceCloseDrawer()"></div>
        <div class="s-drawer" role="dialog" aria-label="Workspace settings">
            <div class="s-drawer-head">
                ...
                <button class="s-drawer-close"
                        onclick="window.__salienceCloseDrawer && window.__salienceCloseDrawer()"
                        aria-label="Close settings">✕</button>
            </div>
        </div>
        """, unsafe_allow_html=True)

        # Drawer body uses Streamlit widgets for real interactivity
        st.markdown('<div style="max-width:320px;margin-left:auto;padding:20px 0 0">', unsafe_allow_html=True)
        ...
        st.markdown('<span class="s-drawer-label" style="display:block;margin-top:16px">Language matrix</span>', unsafe_allow_html=True)
        ...
        st.markdown('<span class="s-drawer-label" style="display:block;margin-top:16px">API credentials</span>', unsafe_allow_html=True)
        if has_cloud_groq and has_cloud_gemini:
            st.markdown("""
            <div class="s-vault-status">
                🔒 Vault active — credentials pre-loaded
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            if new_groq != st.session_state.groq_api_key_input:
                st.session_state.groq_api_key_input = new_groq
            if new_gemini != st.session_state.gemini_api_key_input:
                st.session_state.gemini_api_key_input = new_gemini

        st.markdown('<span class="s-drawer-label" style="display:block;margin-top:20px">Focus mode</span>', unsafe_allow_html=True)
        focus_label = "Exit focus mode" if st.session_state.focus_mode else "Enable focus mode"
        if st.button(focus_label, use_container_width=True, key="drawer_focus_btn"):
            st.session_state.focus_mode = not st.session_state.focus_mode
            st.session_state.show_drawer = False
            st.rerun()
        ...
        if st.session_state.transcript:
            st.divider()
            if st.button("New consultation", use_container_width=True, key="drawer_new"):
...
# =====================================================================
groq_api_key = (
    st.session_state.groq_api_key_input.strip()
    or st.secrets.get("groq_api_key", "")
)
gemini_api_key = (
    st.session_state.gemini_api_key_input.strip()
...
# =====================================================================
# 9. INPUT AREA (hidden in focus mode after analysis)
# =====================================================================
show_input = not st.session_state.transcript or (
    not st.session_state.focus_mode and not st.session_state.transcript
)
...
if not st.session_state.transcript:
    ...
    if st.button(
        ...
        if not groq_api_key or not gemini_api_key:
            st.error("API credentials missing. Open Settings (top right) to add keys.")
        else:
...
if st.session_state.transcript:
    ...
    # ── FOCUS MODE — stripped interface ──
    if st.session_state.focus_mode:
        focus_col1, focus_col2 = st.columns([1, 1], gap="large")
